 ...
 * We use functions to perform actions on data.  Functions are First Class objects
 * in Java Script, meaning they can be used as a value, just like any other data type.
 * This also means they can be passed as an argument or returned from another function.
 * Functions are the Actions in JS that drives our program when given instruction.
**/
...
/** Closures - when a function accesses a variable outside of its scope, 
 * it creates a closure.  A closure gives access to an outer scope from an inner
 * scoped function 
**/

function doo(x) {
    function something(y){
        return x * y;         // Here, the function something is referencing x outside of its scope
    }                         // This is a closure.
}